...
class App {
    constructor() {
        ...
        this.uiManager.onStart = (name, type) => {
            this.initGame(name, type);
        };
        
        const originalTick = this.loop.tick.bind(this.loop);
        this.loop.tick = () => {
            if(this.localPlayer) {
                const obstacles = this.networkManager.getObstacles();
                this.localPlayer.setObstacles(obstacles);
            }
            originalTick();
        };
    ...
    initGame(name, type) {
        if (type === 'cesium') {
            this.localPlayer = new CesiumMan(this.scene, this.inputManager, false);
        } else {
            this.localPlayer = new Fox(this.scene, this.inputManager, false);
        }
        ...
        this.loop.add(this.inputManager); // 保持在最后
